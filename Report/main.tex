...
We also verify the working of this protocol for werner states with initial fidelity $F > 0.5$. We a more simplified circuit for the protocol without the stabilizer operations as given in \cite{paper2} as we only really need to analyse fielity improvement and not how a werner state is created. We run this cicuit using an input werner state with fidelity values greater than 0.5 and measure the output fidelity. We also keep track of the success probability of the protocol. The results are as follows:
